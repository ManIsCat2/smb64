...
--modfied by maniscat2 to fit this romhack
...
if not DEBUG then
    ...
    local levels = {
        -- add levels here
        -- [LEVEL_SA] = { 2,  true,  1560 },
        -- [LEVEL_SL] = { -1, false, 840  },

        -- [LEVEL_XP] = { fixcam, x? (boolean), magic value },
    }

    for doLevel = LEVEL_NONE, LEVEL_COUNT - 1 do
        levels[doLevel] = { 3, false, 128 }
    end
    ...
    function twodee(m)
        ...
        gLakituState.curPos.z = 1028
        --djui_chat_message_create(""..gLakituState.curPos.x)
        local np = gNetworkPlayers[m.playerIndex]
    ...
    hook_event(HOOK_ON_WARP, levelinit)
